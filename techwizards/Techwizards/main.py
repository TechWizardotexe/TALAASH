...
def admin_pic_find():
    name = face()
    encode()
    aadhar_data = aadhar.find_one({"aadhar": name})
    return render_template("details_admin.html", Name=aadhar_data["name"], Contact=aadhar_data["Contact"],
                           Address=aadhar_data["Address"], data_img=aadhar_data["img"].decode("UTF-8"), aadhar=name)
...
def admin_finger_find():
    name = finger()
    aadhar_data = aadhar.find_one({"aadhar": name})
    if aadhar_data is not None:
        return render_template("details_admin.html", Name=aadhar_data["name"], Contact=aadhar_data["Contact"],
                           Address=aadhar_data["Address"], data_img=aadhar_data["img"].decode("UTF-8"))
    else:
...
def found_form():
    ...
    data = request.form
    # print(data)
    aadhar_data = fir.find_one({"Aadhar": local_name})
    doc = {
        ...
        "Contact": aadhar_data["Contact"],
        "img": aadhar_data["img"]
    }
    # print(doc)
    pending_found.insert_one(doc)
    return render_template("details.html", Name=aadhar_data["name"], Contact=aadhar_data["Contact"], police=aadhar_data["police"], Address=aadhar_data["Address"], data_img=aadhar_data["img"], info_name=aadhar_data["informant_name"], info_relation=aadhar_data["informant_relation"])
...
def finger():
    sample = cv2.imread("C:\\Users\\milin\\Desktop\\techhacks\\Hackathon\\techwizards\\Techwizards\\471407501308.jpg")
...
def after_find():
    ...
    fir_data = fir.find_one({"Aadhar": aadhar_data["aadhar"]})
    if fir_data is None:
        NAME = None
